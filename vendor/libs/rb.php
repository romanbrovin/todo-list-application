...
abstract class AQueryWriter
{
	...
	{
		[ $sourceTable, $destTable, $linkTable, $sourceCol, $destCol ] = $this->getRelationalTablesAndColumns( $sourceType, $destType );
		...
		foreach ( $bean as $property => $value ) {
			...
			if ( $value instanceof OODBBean ) {
				...
				if ( strpos( $property, 'own' ) === 0 ) {
					[ $ownAdditions, $ownTrashcan, $ownresidue ] = $this->processGroups( $originals, $value, $ownAdditions, $ownTrashcan, $ownresidue );
					$listName = lcfirst( substr( $property, 3 ) );
					...
					[ $sharedAdditions, $sharedTrashcan, $sharedresidue ] = $this->processGroups( $originals, $value, $sharedAdditions, $sharedTrashcan, $sharedresidue );
					unset( $bean->$property );
				}
			}
		}
...
use RedBeanPHP\Adapter as Adapter;use RedBeanPHP\Adapter\DBAdapter as DBAdapter;
...
use RedBeanPHP\OODBBean as OODBBean;use RedBeanPHP\ToolBox as ToolBox;
...
use RedBeanPHP\BeanHelper as BeanHelper;use RedBeanPHP\Facade as Facade;
    use RedBeanPHP\OODBBean as OODBBean;
...
use RedBeanPHP\BeanHelper as BeanHelper;use RedBeanPHP\BeanHelper\SimpleFacadeBeanHelper as SimpleFacadeBeanHelper;
    use RedBeanPHP\OODBBean as OODBBean;
...
use RedBeanPHP\Observable as Observable;use RedBeanPHP\Observer as Observer;
    use RedBeanPHP\OODBBean as OODBBean;
...
class Facade
{
	...
	*/
     ...
     * [27.02.2023] Roman Brovin
     * @param $cell
     * @param $table
     * @param $id
     * TODO: findCell() нужен ли этот метод?
     * @return Cursor|array|int|string|null
     */
    public static function findCell($cell, $table, $id): Cursor|array|int|string|null
    {
        $query = "
            SELECT $cell
            FROM $table
     ...
     * @param $table
     * @param $id
     * TODO: findCells() нужен ли этот метод?
     * @return array|int|Cursor|NULL
     */
    public static function findCells($cells, $table, $id): Cursor|array|int|null
    {
        $query = "
            SELECT $cells
            FROM $table
...
use RedBeanPHP\OODB as OODB;use RedBeanPHP\OODBBean as OODBBean;
    use RedBeanPHP\RedException as RedException;
...
class DispenseHelper
{
	...
	{
        // Roman Brovin
        // [a-z0-9] было. Добавил _ чтобы были таблицы m_table
...
use RedBeanPHP\Adapter as Adapter;use RedBeanPHP\RedException as RedException;
...
use RedBeanPHP\Finder;use RedBeanPHP\OODBBean as OODBBean;use RedBeanPHP\ToolBox as ToolBox;
...
use RedBeanPHP\OODB;use RedBeanPHP\OODBBean;use RedBeanPHP\QueryWriter;
...
namespace RedBeanPHP\Util {
use RedBeanPHP\Facade as R;use RedBeanPHP\OODBBean;
